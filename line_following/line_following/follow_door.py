...
"""Go towards an entrance / exit door
Doors are indicated by orange or green blobs
Follow_door.py subscribes to /door_choice. This topic publishes a string message indicating whether robot should follow line towards "orange" or "green" door. 

We will publish /door_choice topic using ros2 topic pub

Robot will continue to follow yellow line until it detects specified door and stops in front of door .

subscribes to a topic to receive door color choices
process camera images to detect & track yellow lines 
instructs robot to navigate towards appropriate door based on received door color choice. 
...
class DoorFollower(Node):
    ...
    def camera_callback(self, msg: Image) -> None:
        ...
        # we will generate two cropped imahes one for yellow lane and other for identifying doors
        ...
        # detect yellow contours such that robot can follow yellow lines to move around museum
        # conditional blocks allow robot to selectively detect track contours of 'orange' or 'green'
        # based on door color choice /door_choice
        ...
        # This step is useful for later in order to retrieve the appropriate centroid that will be used in order to coordinate the robot's movement 
        ...
        # guarantees that if no yellow lines are detected but a door of specified color is detected, robot halts its movement along yellow lines and shifts its focus to navigating toward detected door. 
        # indicates robot has reached its ultimate destination.
        ...
        cv2.imshow("RES", res)
        cv2.waitKey(1)
        # This error is then used to determine the robot's motion strategy, by adjusting its angular velocity to keep the centroid aligned with the center of the image.
